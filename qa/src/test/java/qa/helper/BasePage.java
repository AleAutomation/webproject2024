...
public class BasePage {
	...
	public boolean obterPosicaoElemento(By by, int posicaoA, int posicaoB) {
        WebElement meuElemento = DriverFactory.getDriver().findElement(by);

        // Obtenha as coordenadas X e Y do elemento
        int posicaoX = meuElemento.getLocation().getX();
        int posicaoY = meuElemento.getLocation().getY();

        // Verifique se o elemento está na posição esperada
        if (posicaoX == posicaoA && posicaoY == posicaoB) {
            return true;
        } else {
            return false;
        }
